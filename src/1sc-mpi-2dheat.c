...
#define DEBUG_MVP(fmt, args...) do {         \
    int rank = 0;                               \
    MPI_Comm_rank(MPI_COMM_WORLD, &rank);       \
    fprintf(stdout, "[rank_%d][%s][%s] "fmt,    \
            rank, __FILE__, __func__, ##args);  \
} while(0);
...
int main(int argc, char** argv) {
  ...
  float **U_Next;
  /* helper variables */
  double convergence,convergence_sqd,local_convergence_sqd;
  /* available iterator  */
  int i,j,k,m,n; 
...
*/
  ...
  while (1) {
    ...
    /* broadcast method to use */
    (void) MPI_Bcast(&convergence,1,MPI_DOUBLE,0,MPI_COMM_WORLD);
...
void gauss_seidel (float **current_ptr, float **next_ptr){
    ...
    int i, j, p, my_rank, my_start, my_end, my_num_rows;
    float U_Curr_Above[(int)floor(WIDTH / H)];  /* 1D array holding values from the bottom row of PE above */
    float U_Curr_Below[(int)floor(WIDTH / H)];  /* 1D array holding values from the top row of PE below */
    float U_Send_Buffer[(int)floor(WIDTH / H)]; /* 1D array holding values to send */
    float W = 1.0;
    ...
    /* Create windows for remote memory access */
    MPI_Win_create(U_Send_Buffer, (int)floor(WIDTH / H) * sizeof(float), sizeof(float), MPI_INFO_NULL, MPI_COMM_WORLD, &win); 
    MPI_Win_lock_all(MPI_MODE_NOCHECK, win);
    
    /* Communicate ghost rows */
    if (p > 1) {
        /* Communicate bottom row */
        if (my_rank < (p - 1)) {
            /* Populate send buffer with bottom row */
            for (i = 0; i < (int)floor(WIDTH / H); i++) {
                U_Send_Buffer[i] = current_ptr[my_num_rows - 1][i];
            }
            /* Lock the target's window, push data, and unlock
             * */
          //  MPI_Win_lock(MPI_LOCK_EXCLUSIVE, my_rank + 1, 0, win);
            MPI_Get(U_Curr_Above, (int)floor(WIDTH / H), MPI_FLOAT, my_rank + 1, 0, (int)floor(WIDTH / H), MPI_FLOAT, win);
            MPI_Win_flush(my_rank+1, win);
            MPI_Win_flush(my_rank, win);
           // MPI_Win_unlock(my_rank + 1, win);
        }
        ...
        if (my_rank > ROOT) {
            ...
            /* Lock the target's window, push data, and
             * unlock */
          //  MPI_Win_lock(MPI_LOCK_EXCLUSIVE, my_rank - 1, 0, win);
            MPI_Get(U_Curr_Below, (int)floor(WIDTH / H), MPI_FLOAT, my_rank - 1, 0, (int)floor(WIDTH / H), MPI_FLOAT, win);
            MPI_Win_flush(my_rank-1, win);
            MPI_Win_flush(my_rank, win);
         //   MPI_Win_unlock(my_rank - 1, win);
        } 
...
//    MPI_Win_free(&win_below);
 ...
 void gs (float ** current_ptr,float ** next_ptr) {
    int i,j,p,my_rank,my_start,my_end,my_num_rows;
    float *U_Curr_Above; //[(int)floor(WIDTH/H)];  
    /* 1d array holding values from bottom row of PE above */
    float *U_Curr_Below; //[(int)floor(WIDTH/H)];  
    /* 1d array holding values from top row of PE below */
    float U_Send_Buffer[(int)floor(WIDTH/H)]; 
    /* 1d array holding values that are currently being sent */
    float W =  1.0;

    MPI_Win win_above, win_below;
    //float *win_buff; 
    ...
    MPI_Win_allocate((int)floor(WIDTH/H), sizeof(float), 
            MPI_INFO_NULL, MPI_COMM_WORLD, &U_Curr_Above, &win_above);
    MPI_Win_allocate((int)floor(WIDTH/H), sizeof(float), 
            MPI_INFO_NULL, MPI_COMM_WORLD, &U_Curr_Below, &win_below);

    
    //MPI_Win_lock_all(MPI_MODE_NOCHECK, win_above);
    //MPI_Win_lock_all(MPI_MODE_NOCHECK, win_below);
    ...
    if (p > 1) {
        ...
        if (my_rank < (p-1)) {
            ...
            /* non blocking send */
            MPI_Win_lock(MPI_LOCK_EXCLUSIVE, my_rank+1, 0, win_below);
            MPI_Put(U_Send_Buffer, (int)floor(WIDTH/H), MPI_FLOAT, my_rank+1,0,
                    (int)floor(WIDTH/H), MPI_FLOAT, win_below); // versus win_below;
            MPI_Win_unlock(my_rank+1, win_below);
          //  MPI_Win_flush(my_rank+1, win_above);
          //  MPI_Win_flush(my_rank, win_above);
        ...
        //memcpy(win_temp, U_Curr_Above, (int) floor(WIDTH/H)*sizeof(float));

        /* send/receive top rows */
        //U_Curr_Below = win_buff;
        if (my_rank > ROOT) {
            ...
            /* non blocking send */
            MPI_Win_lock(MPI_LOCK_EXCLUSIVE, my_rank-1, 0, win_above);
            MPI_Put(U_Send_Buffer, (int)floor(WIDTH/H), MPI_FLOAT, my_rank-1,
                    0, (int)floor(WIDTH/H), MPI_FLOAT, win_above);
            MPI_Win_unlock(my_rank-1, win_above);
            //MPI_Win_flush(my_rank-1, win_below);
            //MPI_Win_flush(my_rank, win_below);
    ...
    //MPI_Win_fence(0, win_above);
    //MPI_Win_fence(0, win_below);
   ...
   /* solve next blacks (i+j) even .... using next reds */
    ...
    MPI_Win_unlock_all(win_above);
    MPI_Win_unlock_all(win_below);
    
    //free (win_temp);
    //free (win_buff);
    MPI_Win_free(&win_above);
    MPI_Win_free(&win_below);
 }
